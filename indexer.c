...
*/
static indexer_entry_record_t *get_indexer_entry_record(indexer_entry_t *indexer_entry, char *file_path) {
...
static int record_compare_function(void *first, void *second) {
    ...
    int val1 = first_record->count;
    int val2 = second_record->count;
    printf("%i -> %i\n", val1, val2);
...
*/
static indexer_entry_t *get_indexer_entry(indexer_t *indexer, char *token) {
...
static void handle_token(indexer_t *indexer, char *file_path, char *token) {
    ...
    record->count++;
    //TODO: We need a way of resorting these after..
}
